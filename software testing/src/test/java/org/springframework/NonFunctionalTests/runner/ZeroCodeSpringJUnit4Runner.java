package org.springframework.NonFunctionalTests.runner;
...
public class ZeroCodeSpringJUnit4Runner extends SpringJUnit4ClassRunner {
	...
	private void prepareResponseReport(String logPrefixRelationshipId) {
		...
		logCorrelationshipPrinter.aResponseBuilder().relationshipId(logPrefixRelationshipId).responseTimeStamp(timeNow);
	...
	private String prepareRequestReport(Description description) {
		...
		logCorrelationshipPrinter.aRequestBuilder().stepLoop(0).relationshipId(logPrefixRelationshipId)
				.requestTimeStamp(timeNow).step(stepName);
